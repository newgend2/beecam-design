...
export function createViewer(host, labelsHost, onSelect) {
  ...
  function partGroup(id,pos,explode){const g=new THREE.Group();g.position.copy(v(pos));g.userData={id,base:v(pos),explode:v(explode)};root.add(g);registry.push(g);return g;}
  ...
  function bolt(parent,position,axis){
    ...
    const head=mesh(new THREE.CylinderGeometry(4.66,4.66,2.63,20),black,g,[0,1.315,0]);head.userData.id='screw';
    const socket=mesh(new THREE.CylinderGeometry(1.65,1.65,.12,6),mat(0x0a1015),g,[0,2.67,0]);socket.userData.id='screw';
    const shaft=mesh(new THREE.CylinderGeometry(2.5,2.5,8,12),black,g,[0,-4,0]);shaft.userData.id='screw';
    const nut=box(g,[9,3,9],[0,-6,0],nutMat);nut.userData.id='nut';
  }
  ...
  for(const g of registry)g.traverse(m=>{if(m.isMesh){m.userData.id??=g.userData.id;m.userData.baseColor=m.material.color.clone();clickables.push(m);}});
  let selected=null,labels=true,hardware=true,explode=0,isolated=false;
  function updateVisibility(){for(const g of registry){const hw=!locations[g.userData.id];g.visible=(!hw||hardware)&&(!isolated||g.userData.id===selected||(selected==='screw'||selected==='nut')&&hw);g.traverse(m=>{if(!m.isMesh)return;m.visible=!isolated||selected!=='screw'&&selected!=='nut'||m.userData.id===selected;});}}
