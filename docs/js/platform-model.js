...
import {panelShape,stopperShape,patternUV} from './platform-shapes.js';

export function addPlatform({partGroup,mesh,bolt,addLabel}){
  const texture=new THREE.TextureLoader().load(new URL('../assets/contrast-pattern.svg',import.meta.url).href);
  ...
  function surface(s,g,offset=[0,0],width=420,height=300){
    const cap=mesh(patternUV(new THREE.ShapeGeometry(s,96),width,height,offset),print,g,[0,0,P.acrylicThickness+.035]);cap.userData.id='pattern';patternSurfaces.push(cap);return cap;
  }
  ...
  const vaneShape=new THREE.Shape([[-52,0],[52,0],[65,35],[65,159],[47,224],[43,240],[-43,240],[-47,224],[-65,159],[-65,35]].map(p=>new THREE.Vector2(...p)));
  ...
  addLabel('crossVanes',vanes,[25,310,cz],[58,-20]);
