import drawingIndex from './drawing-index.js?v=e0cc66a16aeb';
import {parts,totalLength,sourceURL} from './data.js?v=e0cc66a16aeb';
import {createViewer} from './model.js?v=e0cc66a16aeb';
...
const drawingPath=p=>p.id==='pattern'?'assets/contrast-pattern.svg':`drawings/${({platformNuts:'nut',boxArmNuts:'nut',boxArmScrews:'screw'}[p.id]||p.id)}.svg`;
...
const drawingParts=Object.keys(names).flatMap(a=>[{id:drawingAssembly[a],code:order[a],name:names[a]+' layout',note:a==='enclosure'?'Lid ports follow the internal plate datums. Shell mouldings and wall thickness are schematic.':a==='internals'?'Board layout, stack heights and cable connections; unmeasured installation heights are marked provisional.':a==='frame'?'Front, side and plan views. The dashed upper-arm location is provisional.':'DXF plan geometry, opening, stepped seam and bolt-hole positions.'},...parts.filter(p=>p.assembly===a)]);
