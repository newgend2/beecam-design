...
for(const p of internalParts.filter(p=>!['piPlate','aiCamera'].includes(p.id))){
 ...
 if(spec){const [w,d,px,py,hole,c]=spec,k=Math.min(5,300/d),x=255-w*k/2,y=165;
  ...
  b=rect(140,170,145,275,'#394950');for(let j=0;j<2;j++)for(let i=0;i<20;i++)b+=rect(179+j*65,180+i*13,12,6,'#d0b761',0);
  b+=text(460,160,'2 rows × 20 pins · 2.54 mm pitch')+text(460,202,'Female socket onto Pi GPIO')+text(460,244,'Male pins through HAT to Witty Pi')+text(460,285,'Total height / pin length: to confirm')+wrap(p.note,460,330);
  ...
  if(standoff)b=rect(175,180,100,180,'#dbc78b')+rect(210,360,30,55,'#dbc78b')+circle(225,125,50,'#dbc78b')+circle(225,125,15)+dv(180,360,132,175,`${p.id==='rtcStandoffs'?'≈':''}${h} body`);
  ...
  else b=rect(100,216,42,64,'#bac5ca')+rect(142,233,225,30,'#bac5ca')+text(100,332,'Thread length pending','small');
  b+=text(455,160,`${p.qty} used · ${p.dims}`)+wrap(p.note,455,207,64)+text(455,386,'Verify lengths and thread engagement.','small');
 }
...
b+=text(590,520,'5 mm retainers: Pi at OLED end; Witty at other end.','small')+text(590,550,'15 / 20 / 10 / 5 mm bodies confirmed; RTC pending.','small');
...
b+=text(80,830,'OLED order (screen side, pin edge above): black · red · yellow · blue','small')+text(80,857,'2 cables confirmed. HELIFOUNER brass kit; Kadrick 660-piece M2 hardware kit.','small');
await save('internalsAssembly','04 / Internal electronics assembly','Manufacturer board sizes + supplied acrylic datums; main spacer lengths confirmed',b,930);
